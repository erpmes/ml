...
def compute_discounted_rewards(rewards, gamma=0.99):
    discounted = []            # 存储计算后的折扣奖励
    cumulative = 0             # 累计衰减值初始化
    for r in reversed(rewards):  # 逆序遍历奖励列表（从最后一步到第一步）
        cumulative = r + gamma * cumulative  # 核心公式：当前奖励 + 未来奖励的衰减总和
        discounted.append(cumulative)        # 将当前步的累计值加入列表
    discounted.reverse()       # 反转列表，使顺序与原始奖励一致
    return discounted          # 返回处理后的折扣奖励列表

#​逆序遍历原因：强化学习中未来奖励需向前传播。例如，第t步的折扣奖励需包含t+1步及之后的奖励。
"""
# 逆序循环：
r3: cumulative = r3 + 0.99 * 0 = r3 → discounted = [r3]
r2: cumulative = r2 + 0.99*r3 → discounted = [r3, r2+0.99r3]
r1: cumulative = r1 + 0.99*(r2+0.99r3) → discounted = [r3, r2+0.99r3, r1+0.99r2+0.99²r3]
# 反转后得到： [r1+0.99r2+0.99²r3, r2+0.99r3, r3]
"""
...
for batch in prg_bar:
    ...
    for episode in range(EPISODE_PER_BATCH):
        ...
        episode_rewards = []    # 存储当前episode的即时奖励
        episode_log_probs = []  # 存储当前episode的log概率        
